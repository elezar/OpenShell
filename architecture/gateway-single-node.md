...
On WSL2 hosts, the GPU is exposed through `/dev/dxg` rather than native NVIDIA device nodes. The NVIDIA device plugin generates a CDI spec at `/var/run/cdi/k8s.device-plugin.nvidia.com-gpu.json`, but this spec has two incompatibilities with k3s/containerd: an unsupported `cdiVersion` value and a device named `"all"` instead of the expected numeric index `"0"`. The cluster entrypoint runs a background `inotifywait`-based watcher that detects these specs and writes a corrected version to `/var/run/cdi/openshell-wsl.json`. See [WSL2 CDI spec watcher](#wsl2-cdi-spec-watcher) for implementation details.
